...
def detail_show_zip(request, uid):
	...
	if m:
		...
		try:
			mi, created = MovieInfo.objects.get_or_create(id=uid, defaults={ 'title':m['title'], 'timestamp':datetime.datetime.now()} )
...
def detail(request, uid):
	...
	try:
		mi, created = MovieInfo.objects.get_or_create(id=uid, defaults={'title':'title_occupy', 'timestamp':datetime.datetime.now()})
		if created:
			collection = settings.MONGOINFO
